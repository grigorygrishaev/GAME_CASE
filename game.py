...
class Game:
    ...
    def shopping(self):
        ...
        print('Осталось денег: ', self.money)
        self.ans = input(
            'Время сейчас такое, что голыми руками мало, что можно сделать. Будем покупать кастеты? ').lower()
        if self.ans == 'да':
            while self.t != -1:
                print()
                k = self.money // 20
    ...
    def feeding(self):
        ...
        if self.food >= self.labor:
            ...
            print(
                'Похоже, тебе нечем кормить людей. Хочешь обменять оружие на еду (1 оружие = 2 единицы еды)? У вас есть ',
                self.guns,
...
print(
    '4) Блатной — дела идут в гору, с вашим мнением считаются, уважают за грамотное управление и защиту чести. Дальше — больше.')
